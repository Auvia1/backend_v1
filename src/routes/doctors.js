...
//   try {
//     const { id } = req.params;
//     const { clinic_id, day_of_week, start_time, end_time, slot_duration_minutes, effective_from, effective_to, max_appointments_per_slot } = req.body;
...
//     const result = await pool.query(
//       `INSERT INTO doctor_schedule (clinic_id, doctor_id, day_of_week, start_time, end_time, slot_duration_minutes, effective_from, effective_to, max_appointments_per_slot)
//        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
//        RETURNING *`,
//       [clinic_id, id, day_of_week, start_time, end_time, slot_duration_minutes, effective_from || null, effective_to || null, max_appointments_per_slot || null]
//     );
...
//     const { day_of_week, start_time, end_time, slot_duration_minutes, effective_from, effective_to, max_appointments_per_slot } = req.body;
...
//     if (max_appointments_per_slot !== undefined) { updates.push(`max_appointments_per_slot = $${paramCount++}`); values.push(max_appointments_per_slot); }
...
router.post("/:id/schedule", async (req, res) => {
  try {
    ...
    // Sanitize: ensure every element is an integer, or pass null if empty/missing
    const slotsArray =
      Array.isArray(max_appointments_per_slot) && max_appointments_per_slot.length > 0
        ? max_appointments_per_slot.map((v) => parseInt(v) || 1)
        : null;

    const result = await pool.query(
      `INSERT INTO doctor_schedule
         (clinic_id, doctor_id, day_of_week, start_time, end_time,
          slot_duration_minutes, effective_from, effective_to, max_appointments_per_slot)
...
router.patch("/:id/schedule/:schedule_id", async (req, res) => {
  try {
    ...
    // FIX: cast to integer[] so pg serialises the JS array correctly
    if (max_appointments_per_slot !== undefined) {
      const slotsArray =
        Array.isArray(max_appointments_per_slot) && max_appointments_per_slot.length > 0
          ? max_appointments_per_slot.map((v) => parseInt(v) || 1)
          : null;
      updates.push(`max_appointments_per_slot = $${paramCount++}::integer[]`);
      values.push(slotsArray);
    }
...
router.post("/:id/time-off", async (req, res) => {
  try {
    ...
    if (!clinic_id || !start_time || !end_time) {
      return res.status(400).json({
        success: false,
        error: "clinic_id, start_time, and end_time are required (ISO 8601 format)",
      });
    }
...
router.get("/:id/appointments", async (req, res) => {
  try {
    ...
    let query = `
      SELECT
        a.id, a.clinic_id, a.patient_id, a.doctor_id,
    ...
    let countQuery = `
      SELECT COUNT(*) FROM appointments a
      WHERE a.doctor_id = $1 AND a.clinic_id = $2 AND a.deleted_at IS NULL
    `;
    const countParams = [id, clinic_id];
    let countParamIdx = 3;

    if (status) { countQuery += ` AND a.status = $${countParamIdx++}`; countParams.push(status); }
    if (start_date) { countQuery += ` AND DATE(a.appointment_start AT TIME ZONE 'Asia/Kolkata') >= $${countParamIdx++}`; countParams.push(start_date); }
    if (end_date) { countQuery += ` AND DATE(a.appointment_start AT TIME ZONE 'Asia/Kolkata') <= $${countParamIdx++}`; countParams.push(end_date); }
    if (patient_id) { countQuery += ` AND a.patient_id = $${countParamIdx++}`; countParams.push(patient_id); }
...
router.get("/:id/slots", async (req, res) => {
  try {
    ...
    if (!clinic_id) return res.status(400).json({ success: false, error: "clinic_id query param required" });
    ...
    while (current + slotDuration <= endMins) {
      ...
      const isBooked  = bookedTimes.has(timeStr);
      const isOnLeave = timeOffResult.rows.some((to) => timeStr >= to.off_start && timeStr < to.off_end);
